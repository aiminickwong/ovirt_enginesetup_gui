...
class Plugin(plugin.PluginBase, dialog.DialogBaseImpl):
    ...
    #)
    #def TUI_show():
    #    from TUI_startup import __main__ as main
    #    main.run(self)
    #    self.environment.setdefault('engine_setup_config',[]).append((name,note)) 
    ...
    def queryString(
        ...
        #value=self.context.TUI_configuration.get(name,None) 
        #print "NAME: %s ,VALUE :%s" %(name,value)
        #print "CONFIGURATION LIST %s" %self.context.TUI_configuration
        #if value:
        #    return value
        ...
        if True:
            ...
            while not accepted:
                ...
                elif default and not value and value != default:
    ...
    def queryMultiString(self, name, note=None):
        ...
        while True:
            ...
            value.append(v)
        return value
    #def queryValue_Str(self, name, hint,*note):
    #    self.logger.debug('query %s', name)
    #    hint="("+hint+")" if hint else ''
    #    self.dialog.note(text=hint+note[0])
    #    name = self._readline()
    #    self.dialog.note(text=note[1])
