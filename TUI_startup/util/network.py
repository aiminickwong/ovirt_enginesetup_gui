...
#from ovirt.node import base, config
...
import TUI_startup.util.fs
import TUI_startup.util.process as process
import re
import socket
import struct
"""
...
#                "udev informations are incomplete (udevadm re-trigger?)"
    ...
    hwaddr = utils.fs.get_contents(hwfilename).strip()
    info["hwaddr"] = hwaddr
...
def node_nics():
    ...
    all_cfgs = dict((i,cnetwork.iface(i)) for i in all_ifaces)
