...
class validator_license(Validator):
    ...
    def validate(self, license):
        ...
        try:
            ...
            self.logger.debug("licenseName : %s name : %s" % (licenseName,strmd5(name)[0:4]))
            if (licenseName == strmd5(name)[0:4]):
                self.logger.debug("Param generation passed, value for the questions is valid")
                valid = True
            else:
                valid = False
        except:
